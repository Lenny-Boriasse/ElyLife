...
function zmlab2.Equipment.DrawButton(Equipment,txt,y,IsHovered)
    ...
    if IsHovered then
        ...
        zmlab2.util.DrawOutlinedBox( -110, y-30, 220, 60, 4, zmlab2.colors["white01"])
    end

    draw.SimpleText(txt,font,0,y,zmlab2.colors["white01"],TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
end
...
function zmlab2.Equipment.Repair(Equipment)
    zmlab2.PointerSystem.Start(Equipment,function()
        ...
        if IsValid(zmlab2.PointerSystem.Data.Target) then
            ...
            draw.SimpleText("[" .. val .. "/" .. max .. "]", zmlab2.GetFont("zmlab2_vgui_font01"), data2D.x, data2D.y, zmlab2.colors["white01"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            zmlab2.util.DrawOutlinedBox(data2D.x - 100, data2D.y - 25, 200, 50, 4, zmlab2.colors["white01"])
        end
    end)
end
