...
function zmlab2.Palette.Draw(Palette)
    if zmlab2.util.InDistance(Palette:GetPos(), LocalPlayer():GetPos(), 500) and zmlab2.Convar.Get("zmlab2_cl_drawui") == 1  then
        cam.Start3D2D(Palette:LocalToWorld(Vector(0, 0, 50 + 5 * Palette.Count_Z)), Angle(0, LocalPlayer():EyeAngles().y - 90, 90), 0.1)
            ...
            draw.SimpleText(Palette.Money, zmlab2.GetFont("zmlab2_font02"), 0, 0, zmlab2.colors["white01"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            zmlab2.util.DrawOutlinedBox(-boxSize / 2, -30, boxSize, 60, 4, zmlab2.colors["white01"])
        cam.End3D2D()
    end
end
    ...
    if zmlab2.util.InDistance(LocalPlayer():GetPos(), Palette:GetPos(), 1500) then
        if Palette.UpdateClientProps == true then
            ...
            for k,v in pairs(Palette.MethList) do
                ...
                money = money + zmlab2.Meth.GetValue(v.t,zmlab2.config.Crate.Capacity,v.q)
            end
