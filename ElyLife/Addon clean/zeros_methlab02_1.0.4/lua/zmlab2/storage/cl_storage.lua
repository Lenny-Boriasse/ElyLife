...
function zmlab2.Storage.DrawUI(Storage)
    if zmlab2.util.InDistance(LocalPlayer():GetPos(),Storage:GetPos(), 1000) and zmlab2.Convar.Get("zmlab2_cl_drawui") == 1 then
        cam.Start3D2D(Storage:LocalToWorld(Vector(0, 13.5, 40)), Storage:LocalToWorldAngles(Angle(0, 180, 90)), 0.1)
            ...
            zmlab2.util.DrawOutlinedBox(-barSize / 2, -48, barSize, 48, 2, zmlab2.colors["white01"])
            draw.SimpleText(zmlab2.language["Storage"], zmlab2.GetFont("zmlab2_font02"), 0, -23, zmlab2.colors["white01"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        cam.End3D2D()
    end
end
