...
function MiniGameVGUI:Paint(w, h)
    ...
    if self.IsAnimating == false and self.IsFalling == false then
        draw.SimpleText(zmlab2.language["SPACE"], zmlab2.GetFont("zmlab2_vgui_font01"), w / 2, h * 0.75, zmlab2.colors["white01"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

        local barSize = w * self.ErrorMargin
        if math.abs(self.PosX) < self.ErrorMargin then
            draw.RoundedBox(5, w / 2 - barSize / 2, h - 165 * zmlab2.hM, barSize, 10 * zmlab2.hM, zmlab2.colors["white01"])
        else
