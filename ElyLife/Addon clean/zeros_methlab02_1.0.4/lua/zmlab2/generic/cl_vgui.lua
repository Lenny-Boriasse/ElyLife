...
function VGUIItem:Init()
    ...
    TopContainer.Paint = function(s, w, h)
        zmlab2.util.DrawOutlinedBox(0 * zmlab2.wM, 0 * zmlab2.hM, w - 55 * zmlab2.wM, h, 2, zmlab2.colors["white01"])
    end
    ...
    TitleBox:SetTextColor(zmlab2.colors["white01"])
...
function VGUIItem:Paint(w, h)
    ...
    zmlab2.util.DrawOutlinedBox(0 * zmlab2.wM, 0 * zmlab2.hM, w, h, 2, zmlab2.colors["white01"])
end
...
function zmlab2.Interface.CreateTooltip(t_mdldata, s_name, s_money, data)
    ...
    tt_img.PostDrawModel = function(s,ent)
        cam.Start2D()
            ...
            zmlab2.util.DrawOutlinedBox(0 * zmlab2.wM, 0 * zmlab2.hM, ToolTipSize, ToolTipSize, 2, zmlab2.colors["white01"])
        cam.End2D()
    end
    ...
    local function AddItem(txt01, font, _dock, align)
        ...
        pnl:Dock(_dock)
        pnl:SetText(txt01)
        pnl:SetTextColor(zmlab2.colors["white01"])
        pnl:SetFont(font)
    ...
    local function AddItemBox(txt01,txt02,txt02_color)
        ...
        pnl.Paint = function(s, w, h)
            ...
            draw.SimpleText(txt01,font, 10 * zmlab2.wM, h / 2, zmlab2.colors["white01"], TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    ...
    if data.filter_time and data.mix_time and data.vent_time then
        ...
        AddItemBox(zmlab2.language["Duration"], zmlab2.util.FormatTime(time), zmlab2.colors["white01"])
    end
        ...
        itm.Paint = function(s, w, h)
            if b_isLocked == false then
                ...
                if IsValid(LocalPlayer().zmlab2_Tent) and LocalPlayer().zmlab2_Tent:GetColorID() == k then
                    zmlab2.util.DrawOutlinedBox(0, 0, w, h, 2, zmlab2.colors["white01"])
                end
...
function zmlab2.Interface.AddModelList(pnl,list,isLocked,isSelected,onClick,getData,prePopulate)
    ...
    if table.Count(list) > 8 then
        ...
        MainContainer.Paint = function(s, w, h)
            //draw.RoundedBox(0, 0, 0, w, h, zmlab2.colors["white01"])
        end
    else
    ...
    for k, v in ipairs(list) do
        ...
        Button.Paint = function(s, w, h)
            if s.IsLocked then
                ...
                draw.SimpleText(s_name, font, 7 * zmlab2.wM,4 * zmlab2.hM, zmlab2.colors["white01"], TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
            end
