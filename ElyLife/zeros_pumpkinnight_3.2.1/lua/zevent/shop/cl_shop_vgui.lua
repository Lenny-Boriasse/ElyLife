...
function zpn.vgui.ShopInterface.Action_BuyItem()
	if zpn_Shop_SelectedItem == nil then return end
	if zpn.config.Shop[zpn_Shop_SelectedItem] == nil then return end
	if zpn.config.Shop[zpn_Shop_SelectedItem].type == nil then return end
	if zpn.PurchaseType[zpn.config.Shop[zpn_Shop_SelectedItem].type] == nil then return end

	if zpn.PurchaseType[zpn.config.Shop[zpn_Shop_SelectedItem].type](LocalPlayer(),zpn.config.Shop[zpn_Shop_SelectedItem]) ~= true then return end
...
function zpn.vgui.ShopInterface.HelpPage(parent)
	...
	cmd_title:SetText("Commandes Chat")
    ...
    AddTextField("!discord",zpn.language.General["cmd_discord"])
...
function zpn.vgui.ShopInterface.AddItemContent(parent,i)

	local itemData = zpn.config.Shop[i]

	// Is it a easyskin?
	if itemData.skin_name then
		...
		zpn_ShopItems[i].EasySkinPanel = vgui.Create("DImage", parent)
		zpn_ShopItems[i].EasySkinPanel:SetSize(parent:GetWide(), parent:GetTall())
		zpn_ShopItems[i].EasySkinPanel:SetPos(0 * zclib.wM, 0 * zclib.hM)
		zpn_ShopItems[i].EasySkinPanel:SetAutoDelete(true)
		zpn_ShopItems[i].EasySkinPanel:SetVisible(true)
		zpn_ShopItems[i].EasySkinPanel:SetMaterial(image)
	elseif itemData.model then

		zpn_ShopItems[i].Model = vgui.Create("DModelPanel", parent)
		zpn_ShopItems[i].Model:SetSize(parent:GetWide(), parent:GetTall())
		zpn_ShopItems[i].Model:SetPos(0 * zclib.wM, 0 * zclib.hM)
		zpn_ShopItems[i].Model:SetModel(itemData.model)
		zpn_ShopItems[i].Model:SetAutoDelete(true)
		zpn_ShopItems[i].Model:SetColor(color_white)
		zpn_ShopItems[i].Model.ItemSkin = itemData.model_skin
		zpn_ShopItems[i].Model.ItemFOV = itemData.model_fov
		zpn_ShopItems[i].Model:SetDirectionalLight( BOX_TOP, color_white )
		zpn_ShopItems[i].Model:SetDirectionalLight( BOX_FRONT, color_white )

		local ang = itemData.model_angle or Angle(0,90,0)
		zpn_ShopItems[i].Model.LayoutEntity = function(self)

			if self.ItemSkin then
				self.Entity:SetSkin(self.ItemSkin)
			end

			self.Entity:SetAngles(ang)

			local size1, size2 = self.Entity:GetRenderBounds(0,0)
			local size = (-size1 + size2):Length()

			self:SetFOV(self.ItemFOV or 25)
...
function zpn.vgui.ShopInterface.ShopList(parent)
	...
	for i = 1, table.Count(zpn.config.Shop) do
		...
		if itemData.name then
			...
			zpn_ShopItems[i].ItemName:SetText(itemData.name)
		...
		if itemData.desc then
			...
			zpn_ShopItems[i].ItemDesc:SetText(itemData.desc)
		...
		zpn.vgui.ShopInterface.AddItemContent(zpn_ShopItems[i].ModelPanel,i)

		if itemData.ranks and table.Count(itemData.ranks) > 0 then
