...
function zmlab2.Filter.Cycle_Started(Filter)
    ...
    zmlab2.Timer.Create(timerid,1,0,function()
        ...
        if Filter.NextError then
            ...
            local ErrorChance = math.random(10)
            if ErrorChance <= zmlab2.Meth.GetDifficulty(Filter:GetMethType()) then
                local NextTime = (filter_time / 3)
...
function zmlab2.Filter.MiniGame_Completed(Filter,Result)
    zmlab2.Debug("zmlab2.Filter.MiniGame_Completed")
    // NOT USED ANYMORE
end
