...
function ITEM:CanMerge( item )
	return self.Stackable and self:GetClass() == item:GetClass() and
		self:GetData( "MethType" ) == item:GetData( "MethType" )
end
*/
...
function ITEM:CanPickup(ply, ent)
	return zmlab2.Player.IsOwner(ply, ent)
end
*/
...
function ITEM:Use( pl )
	...
	local PotionID = zmlab2.config.Potion_ListID[self:GetData("PotionID")]
	zmlab2.Potion.Use(PotionID,pl,pl:GetPos(),nil)
