...
function zmlab2.Interface.DrawIngredient(x, y, w, h, icon,amount,font)
    ...
    draw.SimpleText(amount .. "x", font, tri_x + 10,y + (h / 2) + 5, zmlab2.colors["white01"], TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
end
...
function zmlab2.Interface.DrawButton(x, y, w, h, txt, hover)
    zmlab2.util.DrawOutlinedBox(x - (w / 2), y - (h / 2), w, h, 2, zmlab2.colors["white01"])
    ...
    draw.SimpleText(txt, font, x, y, zmlab2.colors["white01"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    if hover then
...
function zmlab2.Interface.DrawProgress(Duration,TimePassed,YOffset)
    ...
    draw.SimpleText(math.Clamp(math.Round(time), 0, 100) .. "%", zmlab2.GetFont("zmlab2_font03"), 0, 0 + (YOffset or 0), zmlab2.colors["white01"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end
...
function zmlab2.Interface.Draw(Machine,data)
    ...
    cam.Start3D2D(Machine:LocalToWorld(data.pos), Machine:LocalToWorldAngles(data.ang), 0.05)
        ...
        if Machine.GetErrorStart == nil or Machine:GetErrorStart() < 0 then
            ...
            draw.SimpleText(zmlab2.language["ERROR"], zmlab2.GetFont("zmlab2_font02"), 0, -80, zmlab2.colors["white01"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            ...
            draw.RoundedBox(0, data.w * 0.05 - (data.w / 2), error_height * 0.8, barMax - barSize, 20, zmlab2.colors["white01"])
        end
